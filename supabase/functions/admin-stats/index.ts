...
Deno.serve(async (req) => {
  try {
    ...
    if (payload.role !== 'admin') {
      return new Response(JSON.stringify({ error: 'Admin access only' }), { status: 403 })
    }
    ...
    let ticketQuery = supabase
      .from('tickets')
      .select('fare_charged, issued_at, trip_id, trips(route_id, routes(route_name, base_fare))')
    ...
    const routeMap: Record<string, { route_name: string; revenue: number; tickets_sold: number; free_tickets: number }> = {}
    ...
    for (const t of tickets ?? []) {
      ...
      if (!routeMap[routeName]) {
        routeMap[routeName] = { route_name: routeName, revenue: 0, tickets_sold: 0, free_tickets: 0 }
      }
