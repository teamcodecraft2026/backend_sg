...
Deno.serve(async (req) => {
  try {
    ...
    if (!trip_id) {
      return new Response(JSON.stringify({ error: 'trip_id is required' }), { status: 400 })
    }
    ...
    if (trip.status !== 'scheduled' && trip.status !== 'in_progress') {
      return new Response(JSON.stringify({ error: 'This trip is not available for booking' }), { status: 400 })
    }
