...
Deno.serve(async (req) => {
  try {
    ...
    if (role !== 'conductor') {
      return new Response(JSON.stringify({ error: 'Only conductors can scan tickets' }), { status: 403 })
    }
    ...
    if (ticketError || !ticket) {
      ...
      responseBody = { valid: false, reason: 'Ticket already used', scanned_at: ticket.scanned_at }
      ...
      scan_result = 'invalid'
      responseBody = { valid: false, reason: 'Ticket is not valid for this trip' }
    } else {
    ...
    return new Response(JSON.stringify({ success: true, scan_result, ...responseBody }), {
